...
#' @param size resolution of the test image to be taken
#' @param sigma standard deviation of the noise to add
#' @param smband smoothing band
#' @param tolmult Multiplier to calculate cutoff for branching
#'
#' @export
#'
#' @examples demo2d(size=256,sigma=0.1,smband=10,tolmult=0.001)
demo2d =function(size=256,
               sigma = 0.1,
...
#' @param sigma standard deviation of the noise to add
#' @param smband smoothing band
#' @param tolmult multiplier of tolerance, default is set to 0.01, determines the cutoff for branching
#' @param margin space to leave between the demo image and boundary
#' @param slice_idx slices to plot
#' @param invert Invert image color
#'
#' @returns list of noisy image and smoothed image
#' @export
#'
#' @examples demo3d(size=c(128,128,128),sigma=0.1,smband=4,tolmult=0.01,margin=0.05,slice_idx=NULL,invert=FALSE)
demo3d<- function(size=c(128,128,128),
                  sigma=0.1,
                  smband=4,
                  tolmult=0.01,
                  margin=0.05,
...
#' @param image Integer value represents dimensions of the images, location of a .nii file can be used as a list of predefined images
#' @param sigma standard deviation of the noise to add
#' @param smband smoothing band
#' @param tolmult multiplier of tolerance, default is set to 0.01, determines the cutoff for branching
#' @param margin space to leave between the demo image and boundary
...
#' @param plot_instance how many instances to plot
#'
#' @returns rmse of noisy and smoothed image, sample instances of noisy and smoothed
#' @export
#'
#' @examples demo3diter(image=128,sigma=0.1,smband=4,tolmult=0.01,margin=0.05,slice_idx=NULL,invert=FALSE,n_iter=10,plot_instance=1)
demo3diter <- function(image=128,
                       sigma=0.1,
                       smband=4,
                       tolmult=0.01,
                       margin=0.05,
